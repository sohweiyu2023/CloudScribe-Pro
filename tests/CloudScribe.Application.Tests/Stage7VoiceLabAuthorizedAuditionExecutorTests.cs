...
public sealed class Stage7VoiceLabAuthorizedAuditionExecutorTests
{
    [Fact]
    public async Task ForceFreshPassesExactBoundRequestToAuthorizedExecutor()
    {
        var wav = MinimalPcmWave();
        var executor = new RecordingExecutor(Accepted(wav));
        var coordinator = new VoiceLabAuditionCoordinator(
            _ => Task.FromResult<ReadOnlyMemory<byte>?>(null),
            executor);
    ...
    private sealed class RecordingExecutor(GenerationProviderResponse response) : IVoiceLabAuthorizedAuditionExecutor
    {
        public VoiceLabAuditionRequest? LastRequest { get; private set; }
        public int SubmitCalls { get; private set; }

        public Task<GenerationProviderResponse> SubmitAuthorizedAsync(
