...
$testRoot = Join-Path $env:RUNNER_TEMP 'stage4-batch11-v3-tests'
$packageRoot = Join-Path $env:RUNNER_TEMP 'stage4-batch11-v3-package'
$evidenceRoot = Join-Path $env:RUNNER_TEMP 'stage4-batch11-v3-evidence'
$status = 'failure'
$exitCode = 1
$testedHead = ''
$sourceZipSha = ''
...
function Rematerialize-Staged([string[]]$paths) {
    $env:CLOUDSCRIBE_STAGED_PATHS = ($paths | ConvertTo-Json -Compress)
    @'
import json
import os
import pathlib
import subprocess
paths = json.loads(os.environ["CLOUDSCRIBE_STAGED_PATHS"])
for rel in paths:
...
'@ | python -
    $code = $LASTEXITCODE
    Remove-Item Env:CLOUDSCRIBE_STAGED_PATHS -ErrorAction SilentlyContinue
    Need ($code -eq 0) 'Unable to rematerialize working-tree bytes from the verified Git index.'
}

function Write-Locator([string]$value) {
    $locator = Join-Path $env:GITHUB_WORKSPACE 'carrier/.cloudscribe-ci/stage4-batch11-v2/admission-v3-status.txt'
    ...
    git -C carrier add .cloudscribe-ci/stage4-batch11-v2/admission-v3-status.txt
    git -C carrier commit -m "ci: record Batch 11 v3 $value run $env:GITHUB_RUN_ID"
    if ($LASTEXITCODE -eq 0) {
        git -C carrier push origin "HEAD:refs/heads/$carrierBranch"
        Need ($LASTEXITCODE -eq 0) 'Unable to publish Batch 11 v3 admission locator.'
    }
}
...
try {
    ...
    Need (Test-Path $transformPath -PathType Leaf) 'Batch 11 transform is missing.'
    $actualTransformBlob = (git -C carrier hash-object .cloudscribe-ci/stage4-batch11-v2/apply_batch11.py).Trim()
    Need ($actualTransformBlob -eq $transformBlob) "Batch 11 transform blob mismatch: $actualTransformBlob"

    Set-Location (Join-Path $env:GITHUB_WORKSPACE 'source')
    Need ((git rev-parse HEAD).Trim() -eq $baseHead) 'Stage 4 target moved before Batch 11 v3 admission.'
    Need ((dotnet --version).Trim() -eq '10.0.400') 'Wrong .NET SDK; expected exact 10.0.400.'

    python $transformPath .
    Need ($LASTEXITCODE -eq 0) 'Batch 11 deterministic source transform failed.'
    git add -A
    Need ($LASTEXITCODE -eq 0) 'Unable to stage Batch 11 candidate.'
    ...
    Need (@($staged | Where-Object { $_ -match 'packages\.lock\.json$' -or $_ -match '(^|/)(bin|obj|TestResults|__pycache__)(/|$)' }).Count -eq 0) 'Batch 11 contains forbidden generated or lockfile changes.'

    # Canonicalize all staged non-manifest bytes from the Git index first. The initial
    # transform may have been executed on Windows, where text writers can materialize CRLF.
    # Git's staged blobs are the release bytes, so make the working tree match them before
    # regenerating the manifest.
    Rematerialize-Staged $staged

    python tools/update_sha256_manifest.py
    Need ($LASTEXITCODE -eq 0) 'Unable to regenerate Batch 11 manifest from canonical staged bytes.'
    git add SHA256SUMS.txt
    Need ($LASTEXITCODE -eq 0) 'Unable to stage canonical Batch 11 manifest.'

    $staged = Get-StagedPaths
    Need ($staged.Count -eq $expectedChanged) "Expected $expectedChanged canonical staged paths; found $($staged.Count)."
    Rematerialize-Staged @('SHA256SUMS.txt')
    git diff --exit-code
    Need ($LASTEXITCODE -eq 0) 'Batch 11 canonical working tree differs from the staged Git index.'
    git diff --cached --check
    Need ($LASTEXITCODE -eq 0) 'Batch 11 canonical staged whitespace check failed.'
    python tools/update_sha256_manifest.py --check
    Need ($LASTEXITCODE -eq 0) 'Batch 11 canonical manifest check failed.'

    $fingerprint = Get-IndexFingerprint $staged
    Write-Host "CLOUDSCRIBE_STAGE4_BATCH11_V3_INDEX_FINGERPRINT_SHA256=$fingerprint"
    Need ($fingerprint -eq $expectedFingerprint) "Batch 11 v3 canonical index fingerprint mismatch: $fingerprint"
    ...
    for ($i = 0; $i -lt $projects.Count; $i++) {
        ...
        dotnet test $projects[$i] -c Release --no-build --no-restore -m:1 -nodeReuse:false -p:UseSharedCompilation=false --results-directory $dir --logger 'trx;LogFileName=batch11-v3.trx'
    ...
    Write-Host "CLOUDSCRIBE_STAGE4_BATCH11_V3_DOTNET_TESTS total=$total passed=$passed failed=$failed skipped=$skipped"
    ...
    Need ($fingerprintAfter -eq $expectedFingerprint) "Batch 11 v3 index fingerprint drifted after tests: $fingerprintAfter"

    $remote = (git ls-remote --heads origin "refs/heads/$targetBranch" | Select-Object -First 1)
    $remoteHead = ($remote -split '\s+')[0].Trim()
    Need ($remoteHead -eq $baseHead) "Stage 4 target moved before Batch 11 v3 publication: $remoteHead"

    git config user.name cloudscribe-ci
    git config user.email actions@github.com
    git commit -m 'feat: add Stage 4 pricing plan contracts batch 11'
    Need ($LASTEXITCODE -eq 0) 'Batch 11 v3 commit failed.'
    $testedHead = (git rev-parse HEAD).Trim()
    git push origin "${testedHead}:refs/heads/$targetBranch"
    Need ($LASTEXITCODE -eq 0) 'Unable to publish exact tested Batch 11 v3 source.'
    Write-Host "CLOUDSCRIBE_STAGE4_BATCH11_V3_TESTED_HEAD=$testedHead"

    New-Item -ItemType Directory -Force $packageRoot | Out-Null
    python tools/create_source_archive.py --output-directory $packageRoot
    Need ($LASTEXITCODE -eq 0) 'Deterministic Batch 11 v3 source archive creation failed.'
    ...
    Write-Host "CLOUDSCRIBE_STAGE4_BATCH11_V3_SOURCE_ZIP_SHA256=$sourceZipSha"
    ...
    [IO.File]::WriteAllText((Join-Path $evidenceRoot 'BATCH11-V3-EVIDENCE.txt'), $evidence + "`n", [Text.UTF8Encoding]::new($false))

    $status = 'success'
    $exitCode = 0
}
catch {
    Write-Host "BATCH11_V3_FAILURE: $($_.Exception.Message)" -ForegroundColor Red
    $status = 'failure'
    $exitCode = 1
}
finally {
    try {
        ...
        Write-Host "BATCH11_V3_LOCATOR_FAILURE: $($_.Exception.Message)" -ForegroundColor Red
        $exitCode = 1
    }
}
